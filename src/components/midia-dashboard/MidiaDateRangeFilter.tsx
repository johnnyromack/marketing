...
interface MidiaDateRangeFilterProps {
  dateRange: DateRange;
...
export const MidiaDateRangeFilter = ({
  dateRange,
  onDateRangeChange,
  comparisonRange,
  onComparisonRangeChange,
  showComparison = true,
}: MidiaDateRangeFilterProps) => {
  const [preset, setPreset] = useState<string>('currentCycle');
      ...
      <Popover>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {formatDateRange(dateRange) || "Selecionar"}
          </Button>
        </PopoverTrigger>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      "bg-card min-w-[150px] justify-start text-left font-normal",
                      !comparisonRange?.from && "text-muted-foreground"
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {comparisonRange ? formatDateRange(comparisonRange) || "Anterior" : "Anterior"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
